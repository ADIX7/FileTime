...
namespace FileTime.Uno
{
    ...
    public sealed partial class App : Microsoft.UI.Xaml.Application
    {
        ...
        private void _window_Activated(object sender, WindowActivatedEventArgs args)
        {
            try
            {
                if (_window.Content is Frame frame
                    ...
                    && mainPage.DataContext is MainPageViewModel viewModel)
                {
                    viewModel.ResetSpecialKeys();
                }
            }
            catch { }
        }
        ...
        private void FocusManager_GotFocus(object sender, Microsoft.UI.Xaml.Input.FocusManagerGotFocusEventArgs e)
        {
            ...
            {
                if (scrollViewer.Parent.GetType() == typeof(DependencyObject))
                {
                    FocusDefaultElement();
                }
            }
            else if (e.NewFocusedElement is FrameworkElement frameworkElement && !frameworkElement.IsTabStop)
            {
                FocusDefaultElement();
            }
        }
        ...
        private void FocusDefaultElement()
        {
            if (_window.Content is Frame frame
                && frame.Content is MainPage mainPage
                && mainPage.FindName("CurrentItems") is FrameworkElement element)
            {
